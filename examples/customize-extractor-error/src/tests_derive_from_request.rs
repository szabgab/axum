...
#[tokio::test]
async fn test_derive_from_request_bad_content_type() {
    let response = create_router()
        .oneshot(
            Request::post("/derive-from-request")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        ...
        BadResponse {
            ...
            origin: String::from("derive_from_request"),
        }
    )
}

#[tokio::test]
async fn test_derive_from_request_good_content_type_no_value() {
    let response = create_router()
        .oneshot(
            Request::post("/derive-from-request")
...
#[tokio::test]
async fn test_derive_from_request_good_content_type_with_value() {
    let response = create_router()
        .oneshot(
            Request::post("/derive-from-request")
                .header(http::header::CONTENT_TYPE, mime::APPLICATION_JSON.as_ref())
